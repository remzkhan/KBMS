...
// The Firebase configuration details you provided are now integrated here.
...
// This uses a default ID for local development.
const appId = 'knowledgebase-saas-local-dev';
// NOTE: For the AI functionality to work, you must set up your Gemini API Key.
// For local development, you can paste it here. For production, use environment variables.
const geminiApiKey = "AIzaSyAEYjREpf-Mm9YmTg01SfUhYreoIzMltl8"; // <-- PASTE YOUR GEMINI API KEY HERE
const BACKEND_URL = "http://localhost:3001"; // The URL of your backend server
...
function App() {
    ...
    const generateArticleFromText = async (text, source) => {
        ...
        if (!geminiApiKey || geminiApiKey === "YOUR_GEMINI_API_KEY") { setError("Gemini API Key is not set. Please add it to the App.js file."); return; }
